...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (ID,(name,(count,avg))): (avg,name,count)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg,name,count) : count>500)
...
# randomSplit을 이용하여 6: 2: 2 비율로 랜덤으로 나눠줌
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                       ...
                       .map(lambda((UserID,MovieID),(Rating1,Rating2)):((Rating1-Rating2)**2)))
...
minError = float('inf') #infinite value
bestRank = -1
bestIteration = -1
for rank in ranks:
...
predictedTestRDD = myModel.predictAll(testForPredictingRDD) # predictAll : 기본제공 함수 - RDD에 있는 모든 값을 예측
...
# TODO: Replace <FILL IN> with appropriate code
#myRatingsModel = ALS.train(trainingWithMyRatingsRDD, bestRank, seed=seed, iter)
myRatingsModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
