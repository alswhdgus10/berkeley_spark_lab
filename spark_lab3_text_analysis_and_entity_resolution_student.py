...
def simpleTokenize(string):
    ...
    """
    return filter(None, re.split(split_regex,string.lower())) #공집합을 제외시키기 위해 None을 추가
...
stopwords = set(sc.textFile(stopfile).collect()) #중요하지 않은 단어 집합
...
def tokenize(string):
    ...
    """
    return [word for word in simpleTokenize(string) if word not in stopwords] #simpleTokenize를 통한 word에서 stopwords가 아닌 것들을 출력
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda line : (line[0], tokenize(line[1]))) #record to token
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda pair : len(pair[1])).reduce(lambda a,b : a+b)

totalTokens = countTokens(amazonRecToToken) + countTokens(googleRecToToken) #모든 token을 count
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1,lambda pair:-1 * len(pair[1])) #sorting한 다름에 take(1)해라 / 역순 정렬을 -1 로 함
...
# #### **TF**
# ####특정 문서에서 단어의 빈도수가 높다면 중요한 단어로 가정한다. 단어들의 빈도수를 본다
...
def tf(tokens):
    ...
    """
    tk_cnt_pair = ([[x,tokens.count(x)] for x in set(tokens)])
    length  = float(len(tokens))
    return dict(map(lambda input : (input[0], input[1]/ length), tk_cnt_pair)) # 값/총단어의수 = 빈도 비율
...
# ####inverse document frequency : 전체 document set에서 이 단어가 그 set을 잘 나타낼수 있는건가는 또 다른 문제다. 전체 다큐먼트 셋에서 다 나오면
# ####그 다큐먼트를 대표하는 단어라고 보기 어렵다.
# ####ex. 'quick'이라는 단어가 전체 다큐먼트 셋에서 몇개의 다큐먼트에 나오는가?? - 나오는 다큐먼트가 적을수록 대표성이 커진다.
# ####값이 작을수록 좋다 => 1/200 (200개의 다큐먼트에서 1번 나왔다) => inverse를 취해서 200으로 표시 ( L / n ) => nomalization을 시키기 도함
# ####TF : 하나의 document 내에서 그 document를 잘 표현하는 단어를 구하는것
...
#collectAsMap : collect()는 central server의 메모리에 데이터를 다 모으는것이고, collectAsMap은 결과는 똑같지만 hashmap 구조로 가져옴
...
# #### 문서마다 tf * idf 로 값을 구해서 vector로 나타낼수 있다. 그 후에 vector 끼리의 유사도를 구한다. 여기서는 cosine similarity를 사용한다. 0 은 유사한것이고 1은 완전 별개의 값이다. threshold를 0.2로 두면 0.2 이상이면 유사하다고 보겠다는 뜻이다.
...
crossSmall = (googleSmall
              .cartesian(amazonSmall) #cartesian product를 구함 - 결과가 40,000개가 나옴
              .cache())
...
#idfsSmallWeight는 자주 쓰는 값이기 때문에 모든 곳에 broadcast 시켜라
...
# 정답 집합 RDD: 200* 200 (40,000개) 으로 줄여놨기 때문에 GoldStandard(1300개)에서 겹치는 부분만 추려냄
# 애초의 1300개는 200개를 뽑아낸것들로 만든게 아니기 때문에 이 과정이 필요함
trueDupsRDD = (sims
               .join(goldStandard))
...
# ### **Part 4: Scalable ER**
# #### 데이터 수가 커졌을 때도 사용할 수 있도록 Scalable하게 만듬 / 위의 방법은 Scalable  하지 않기 때문에
# #### pair를 만들기 때문에 데이터 수가 기하급수적으로 큼, 하지만 대부분의 record는 common 하지않기 때문에 Inverted Index를 이용하여 가공
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda pair : (pair[0], tfidf(pair[1], idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda pair : (pair[0], tfidf(pair[1], idfsFullBroadcast.value)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda pair : (pair[0],norm(pair[1])))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda pair : (pair[0],norm(pair[1])))
...
# 
# #### inverted index : 기존에는 doc당 token을 찾았다면 이번에는 token 당 포함된 doc을 찾는 과정 - 이렇게 하면 나중에 cos sim을 구할 경우가 줄어든다. 
# ##### 기존에는 unique token이 2만이라고 하면 각 doc 당 vector는 2만개여야 한다. 대부분의 값은 0 - 나중에 cos sim을 구할때 다 곱해야 하는 비효율적
# ##### ex. A라는 단어를 가지고 있는 doc은 2개, B도 2개, 이런식으로 찾음
# ##### 우선 (token, amazon doc1), (token, google doc1) pair를 만듬 .. 쭉 token 마다 만듬
# ##### 이렇게 만들어진 것을 amazon inverse  와 google inverse를 join 시킴
# ##### (monitor, (a1,g1)) , (monitor,(a7,g3)) 이렇게 이렇게 나옴 -> 이걸 swap 시키면 ((a1,g1),monitor)로 묶임 , ((a1,g1),pc) 이렇게..
# ##### 이것들을 groupbyKey 하면 ((a1,g1),{monitor,pc}) 요렇게 나옴 -> 공통 token list
# ##### 여기서 cos sim을 구하면 ((a1,g1), COS SIM) 이렇게 됨
...
def fastCosineSimilarity(record):
    ...
    value = float(s)/norm(amazonWeightsBroadcast.value[amazonRec])/norm(googleWeightsBroadcast.value[googleRec])
...
# Precision = true-positives / (true-positives + false-positives)- 내가 만든 정답셋의 몇퍼센트나 정답인가? ex. 50%
# Recall = true-positives / (true-positives + false-negatives) - 실제 정답셋의 몇 퍼센트나 맞추는가 ? ex. 33%
# F-measure = 2 x Recall x Precision / (Recall + Precision) - Precision과 Recall을 조합해서 사용하는 값
